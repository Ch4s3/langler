...
- Aim for **80% test coverage** across the codebase to maintain code quality and catch regressions

### After completing a task

**Always** run `mix precommit` when you are done with a task. Fix any issues it reports (compile warnings, test failures, assay/Dialyzer issues, Credo findings) until `mix precommit` passes. The precommit alias runs compile, test with coverage, assay, credo --strict, and format.
