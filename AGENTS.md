...
- **Always** run Credo in strict mode: `mix credo --strict`
- After big changes, run `mix assay --format llm` to check for Dialyzer type issues in a format suitable for LLM analysis
- **Always** run tests after implementing big features to ensure functionality works as expected
