defmodule LanglerWeb.UserLive.TtsSettings do
  @moduledoc """
  LiveView for user TTS settings.
  """
  ...
  def mount(_params, _session, socket) do
    ...
    configs = TtsConfig.get_user_configs(scope.user.id)
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="mx-auto max-w-4xl space-y-8 py-8">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-base-content">TTS Settings</h1>
            <p class="mt-2 text-sm text-base-content/70">
              Configure your Text-to-Speech provider API keys for the listening feature.
            </p>
          </div>
        ...
        <div class="card border border-base-200 bg-base-100 shadow-xl">
          <div class="card-body">
            <h2 class="card-title">Your TTS Configurations</h2>
            ...
            <div :if={@configs == []} class="py-8 text-center">
              ...
              />
              <p class="mt-4 text-base-content/70">No TTS configurations yet.</p>
                  ...
                  <div class="flex-1">
                    <div class="flex items-center gap-2">
                      <h3 class="font-semibold text-base-content">
                        {String.replace(config.provider_name, "_", " ") |> String.capitalize()}
                      </h3>
                    ...
                    <dl class="mt-2 space-y-1 text-sm">
                      <div class="flex gap-2">
                        ...
                        <dd class="font-mono text-base-content/60">
                          {TtsConfig.decrypt_api_key_masked(
                            @current_scope.user.id,
                            config.encrypted_api_key
                          )}
                        </dd>
                      </div>
                      <div :if={config.voice_name} class="flex gap-2">
                        <dt class="font-medium text-base-content/70">Voice:</dt>
                        <dd class="text-base-content">{config.voice_name}</dd>
                      </div>
                    </dl>
                  </div>
                  <div class="flex gap-2">
        ...
        <div class="card border border-base-200 bg-base-100 shadow-xl">
          <div class="card-body">
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-sm text-base-content/80 mb-3">
                    You'll need a Gemini API key from Google AI Studio to use Text-to-Speech.
                  </p>
                  <div class="flex flex-wrap gap-2">
                    ...
                    >
                      <.icon name="hero-key" class="h-4 w-4" /> Get Gemini API Key
                    </.link>
                  </div>
                </div>
              </div>
            ...
            >
              ...
              />
              <p class="-mt-2 text-sm text-base-content/60">
                Leave blank to use the default voice. Gemini-TTS automatically detects language (supports Spanish, English, and many others)
              </p>
  ...
  @impl true
  def handle_event("validate", %{"tts_config" => params}, socket) do
    form = to_form(params, as: :tts_config)
  ...
  def handle_event("validate", params, socket) when is_map(params) do
    ...
    {:noreply, assign(socket, :form, to_form(form, as: :tts_config))}
  end

  def handle_event("save_config", %{"tts_config" => params}, socket) do
    scope = socket.assigns.current_scope

    result =
      if socket.assigns.editing_config do
        config = socket.assigns.editing_config
        TtsConfig.update_config(config, params)
      else
        TtsConfig.create_config(scope.user, params)
      end
    ...
    case result do
      ...
      {:error, changeset} ->
        {:noreply,
         ...
         |> assign(:form, to_form(changeset, as: :tts_config))}
    end
  end

  def handle_event("save_config", params, socket) when is_map(params) do
    handle_event("save_config", %{"tts_config" => params}, socket)
  end

  def handle_event("edit_config", %{"id" => id}, socket) do
    config = TtsConfig.get_config(String.to_integer(id))
  ...
  def handle_event("delete_config", %{"id" => id}, socket) do
    ...
    case TtsConfig.delete_config(config) do
      {:ok, _} ->
        configs = TtsConfig.get_user_configs(scope.user.id)
  ...
  def handle_event("test_config", _, socket) do
    ...
    if form do
      ...
      case TtsConfig.test_config(test_config) do
        {:ok, message} ->
